...
const cancelBooking = async (req, res) => {
    ...
    try {
      ...
      if (nextUser) {
        ...
        // Step 6: Increment the available tickets count for the event
        console.log("No users in the waiting list. Incrementing available tickets.");
        const event = await Event.findByPk(eventId, { transaction });
        if (event) {
          event.availableTickets += 1;
          await event.save({ transaction });
          console.log("Available tickets incremented for event:", event.toJSON());
        } else {
          console.warn(`Event with eventId: ${eventId} not found.`);
        }
      }
  
      // Step 7: Commit the transaction and send success response
      await transaction.commit();
